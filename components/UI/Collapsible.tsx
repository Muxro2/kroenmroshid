...
export default function Collapsible({text} : {text: string}) {
  ...
  function toggleCollapse() {
	...
	return(
		...
		<button className="w-full p-4 flex justify-between text-lg font-bold text-left " onClick={() => toggleCollapse()}>
			<h1>{text.toUpperCase()}</h1>
					<h1>v</h1>
		</button>
			
			<div className={`${collapse? "h-0" : "h-50"} relative w-full  duration-300`}>
				<p className="absolute inset-2 text-md overflow-hidden">
